...
if (fs.existsSync(deployConfig)) {
  const configFromFile: DeployConfig = JSON.parse(fs.readFileSync(deployConfig, 'utf8'))
  ...
  while (!readyToProceed) {
    ...
    switch (existingConfigChoice) {
      ...
      case 'e':
        ...
        break
      default:
        console.log(formatText('Invalid Input!', { bold: true, backgroundColor: 'bg-red', textColor: 'white' }))
        break
    }
  }
}
...
if (requirementsMet.toLowerCase() !== 'y') {
  console.log(formatText('Exiting...', { bold: true, backgroundColor: 'bg-red', textColor: 'white' }))
  process.exit(0)
}
if (['UPDATE', 'NEW'].includes(configStyle)) {
  ...
  if (config !== null) {
    ...
    while (!stackNameApproved) {
      ...
      const stackName = prompter(`(${config?.stackName}):`, config?.stackName ?? '')
      if (stackName.length > 0) {
        config.stackName = stackName
        stackNameApproved = true
      } else if (config.stackName === undefined || (config.stackName.length < 1 && stackName.length < 1)) {
    ...
    if (config.env === undefined) {
      console.log(formatText('Do you want to set the deployment AWS Account ID & Region?', { textColor: 'blue' }))
      console.log(formatText('Not typically needed, but useful if you want to persist deployment destintation outside of the CDK context', { italic: true }))
      const addEnv = prompter(formatText('Do you want to proceed? (y/N):', { bold: true }), 'N')
      if (addEnv.toLowerCase() === 'y') {
        addEnvData = true
      }
    }
    if (addEnvData || (((config.env?.account) != null) || ((config.env?.region) != null))) {
      console.log(formatText('Deployment AWS Account ID', { textColor: 'blue' }))
      const accountId = prompter(config?.env?.account !== undefined ? `(${config?.env?.account}):` : 'Account ID:', config?.env?.account ?? '')
      if (config.env == null) {
        config.env = {}
      }
...
if (config !== null) {
  console.log(formatText('Configuration Complete! 💫', { backgroundColor: 'bg-green', textColor: 'white', bold: true }))
