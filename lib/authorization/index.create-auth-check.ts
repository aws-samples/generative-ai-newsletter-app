...
import { type CreateAuthCheckInput } from '../shared/common/types'

const SERVICE_NAME = 'create-auth-check'
...
const checkReadAuthorization = async (input: CreateAuthCheckInput, context: Context): Promise<{ isAuthorized: boolean }> => {
  ...
  const isAuthInput: IsAuthorizedCommandInput = {
    ...
    entities: {
      entityList: [
        ...
        action.getEntityItem(schema, 'CREATE_NEW_ITEM' as string, action.resourceType, { accountId: user.accountId })
      ]
    }
