...
const lambdaHander = async (event: {
  ...
  metrics.addMetric('SubscriberInvocations', MetricUnit.Count, 1);
  ...
  try {
    ...
    if ($('rss').length > 0 && $('rss').attr('version') === '2.0') {
      metrics.addMetric('RSSFeeds', MetricUnit.Count, 1);
      ...
      metrics.addMetric('ATOMFeeds', MetricUnit.Count, 1);
      logger.debug('Found ATOM feed');
      feedType = DataFeedType.ATOM;
    } else {
      metrics.addMetric('InvalidFeedFormat', MetricUnit.Count, 1);
      throw Error(
...
const storeDataFeed = async (dataFeed: DataFeed): Promise<void> => {
  ...
  if (response.$metadata.httpStatusCode !== 200) {
    ...
    metrics.addMetric('DDBPutFailed', MetricUnit.Count, 1);
    throw Error('Error storing datafeed data');
  } else {
    metrics.addMetric('DDBPutSuccessful', MetricUnit.Count, 1);
  }
};
...
const startIngestionStepFunction = async (
  ...
  if (response.$metadata.httpStatusCode !== 200) {
    ...
    metrics.addMetric('SFNStartFailed', MetricUnit.Count, 1);
    throw Error('Error starting ingestion step function');
  } else {
    metrics.addMetric('SFNStartSuccessful', MetricUnit.Count, 1);
  }
};
