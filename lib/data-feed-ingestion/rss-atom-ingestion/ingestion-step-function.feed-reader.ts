...
const lambdaHandler = async (event: FeedReaderInput): Promise<Article[]> => {
  ...
  metrics.addMetric('FeedReader', MetricUnit.Count, 1);
...
const parseArticlesFromFeed = async (
  ...
  return feedType === 'RSS'
    ? parseRssFeed(feedContents, dataFeedId, accountId)
    : parseAtomFeed(feedContents, dataFeedId, accountId);
};
  ...
  for (const article of $('item')) {
    try {
      ...
      if (url !== null && url.length > 0) {
        ...
        metrics.addMetric('FeedArticleParsed', MetricUnit.Count, 1);
        metrics.addMetric('RSSFeedArticleParsed', MetricUnit.Count, 1);
        logger.debug(
      ...
      metrics.addMetric('FeedArticleParsingFailed', MetricUnit.Count, 1);
    }
  }
  return articles;
};
  ...
  for (const article of $('entry')) {
    try {
      ...
      if (url !== null && url.length > 0) {
        ...
        metrics.addMetric('FeedArticleParsed', MetricUnit.Count, 1);
        metrics.addMetric('ATOMFeedArticleParsed', MetricUnit.Count, 1);
        logger.debug(
