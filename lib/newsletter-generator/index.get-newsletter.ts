...
const lambdaHandler = async (
  ...
  if (newsletterId !== null && newsletterId.length > 0) {
    try {
      ...
      if (newsletter == null) {
        metrics.addMetric('NewsletterNotFound', MetricUnit.Count, 1);
        return null;
      } else {
        metrics.addMetric('NewsletterFound', MetricUnit.Count, 1);
      ...
      metrics.addMetric('ErrorGettingNewsletter', MetricUnit.Count, 1);
...
const getNewsletterDataFeedData = async (
  ...
  try {
    ...
    metrics.addMetric('ErrorGettingNewsletterDataFeeds', MetricUnit.Count, 1);
    return [];
  }
};
