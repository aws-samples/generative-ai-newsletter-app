...
const subscribeCognitoUser = async (
  ...
  try {
    ...
    metrics.addMetric('UserSubscribed', MetricUnit.Count, 1);
    metrics.addMetric('CognitoUserSubscribed', MetricUnit.Count, 1);
  } catch (error) {
...
const subscribeExternalUser = async (
  ...
  try {
    ...
    await dynamodb.send(command);
    metrics.addMetric('UserSubscribed', MetricUnit.Count, 1);
    metrics.addMetric('ExternalUserSubscribed', MetricUnit.Count, 1);
  } catch (error) {
