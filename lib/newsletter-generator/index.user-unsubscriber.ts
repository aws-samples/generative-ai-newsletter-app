...
const unsubscribeCognitoUser = async (
  ...
  try {
    ...
    metrics.addMetric('UserUnsubscribed', MetricUnit.Count, 1);
    metrics.addMetric('CognitoUserUnsubscribed', MetricUnit.Count, 1);
  } catch (error) {
...
const unsubscribeExternalUser = async (
  ...
  try {
    ...
    if (externalUserId === undefined) {
      ...
      await dynamodb.send(command);
      metrics.addMetric('UserUnsubscribed', MetricUnit.Count, 1);
      metrics.addMetric('ExternalUserUnsubscribed', MetricUnit.Count, 1);
    }
  } catch (error) {
