import { TaggedElement, type PromptElementContext, MultiSizeFormattedResponse } from '../prompts/prompt-processing'
...
export class ArticleSummaryBuilder {
  ...
  private readonly articleSummarizationPromptTag = new TaggedElement('articleSummarizationPrompt')
  private readonly response = new MultiSizeFormattedResponse()

  constructor (articleContent: string, articleSummarizationPrompt: string | null) {
  ...
  private getBaseContextPrompt (): PromptElementContext {
    return {
      before: 'You are in charge of summarizing content from articles.' +
  ...
  private getPromptFormattedArticle (): string {
    return this.articleTag.openTag + this.articleContent + this.articleTag.closeTag
  }

  private getPromptFormattedArticleSummarization (): string {
    return this.articleSummarizationPromptTag.openTag +
  ...
  private getFinalInstructionsPrompt (): string {
    return 'The only valid XML tags you can respond with are:\n' +
