...
export class SubscriptionPollStepFunction extends Construct {
  ...
  constructor (
    scope: Construct,
    id: string,
    props: SubscriptionPollStepFunctionProps
  ) {
    super(scope, id)
    ...
    const getSubscriptionsFunction = new NodejsFunction(
      ...
      {
        description:
          'Function responsible for getting all enabled subscriptions to poll',
        handler: 'handler',
        ...
        environment: {
          ...
          NEWS_SUBSCRIPTION_TABLE_TYPE_INDEX:
            props.newsSubscriptionTableTypeIndex
        },
