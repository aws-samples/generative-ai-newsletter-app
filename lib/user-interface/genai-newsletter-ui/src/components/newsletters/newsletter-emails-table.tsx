...
export default function NewsletterEmailsTable() {
  ...
  const getNewsletterEmails = useCallback(async () => {
    // setLoading(true)
    if (!appContext) {
      return
    }
    ...
    const result = await apiClient.newsletters.getNewsletterEmails(
      { newsletterId },
      nextToken
    )
    if (result.errors) {
      ...
      setNewsletterEmails([
        ...(result.data.getNewsletterEmails
      ...
      for (const email of result.data.getNewsletterEmails
        ?.newsletterEmails as NewsletterEmail[]) {
        if (email.htmlPath) {
        ...
        if (email.htmlPath) {
          return (
            <ExpandableSection
              ...
              headerActions={
                email.htmlPath !== null &&
                email.htmlPath !== undefined &&
                email.htmlPath.length > 0 ? (
                  <SpaceBetween size="s" direction="horizontal">
                    <Button
                      onClick={() => {
                        window.open(email.htmlPath as string, '_blank')
                      }}
                    >
            ...
            >
              <Container>
                {newsletterContent[email.emailId] ? (
                  ...
                  <StatusIndicator
                    key={'status-email-' + email.emailId}
                    type="loading"
                  >
                    Loading...
                  </StatusIndicator>
